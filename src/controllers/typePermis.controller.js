...
const TypePermisModel = require("../models/typePermis.models");
...
exports.getTypePermisList = (rep, res) => {
  ...
  TypePermisModel.getTypePermisList((err, typePermis) => {
    ...
    if (typePermis) {
      console.log("typePermis", typePermis);
      res.json({
        status: true,
...
exports.getTypePermisByID = (req, res) => {
  ...
  TypePermisModel.getTypePermisByID(
    req.params.codeTypePermis,
    (err, typePermis) => {
      if (err) res.send(err);
...
exports.createNewTypePermis = (req, res) => {
  ...
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    TypePermisModel.createTypePermis(typePermisReqData, (err, typePermis) => {
      ...
      res.json({
        status: true,
        message: "typePermis ajoutée avec succes",
        data: typePermis.insertcodeTypePermis,
      });
    });
  }
};
...
exports.updateTypePermis = (req, res) => {
  ...
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    TypePermisModel.updateTypePermis(
      req.params.codeTypePermis,
      typePermisReqData,
      (err, typePermis) => {
        if (err) res.send(err);
...
exports.deleteTypePermis = (req, res) => {
  TypePermisModel.deleteTypePermis(
    res.params.codeTypePermis,
    (err, typePermis) => {
      if (err) res.send(err);
